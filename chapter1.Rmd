...
title: 'R''a İlk Adım'
description: "\n210/5000\nBu bölümde, ilk adımlarınızı R ile gerçekleştireceksiniz. Konsolun bir hesap makinesi olarak nasıl kullanılacağını ve değişkenlerin nasıl atanacağını öğreneceksiniz. Ayrıca temel veri türlerini de öğreneceksiniz. Hadi başlayalım!"
---

## R - Datacamp arayüzü nasıl çalışıyor?
...
Sağdaki editörde egzersizleri çözmek için R kodu yazmalısınız. 'Submit Answer' düğmesine bastığınızda, her kod satırı R tarafından çalıştırılır ve kodunuzun doğru olup olmadığına dair bir mesaj alırsınız. R kodunuzun çıktısı, sağ alt köşedeki konsolda gösterilir.

R'da, yorum eklemek için `#` işaretini kullanır, böylece siz ve başkaları R kodunun neyle ilgili olduğunu anlayabilirsiniz. Sadece Twitter gibi! Yorumlar, R kodu olarak çalıştırılmaz, dolayısıyla sonuçlarınızı etkilemez. Örneğin, sağdaki editörde "#Toplama: 1 ile 2'yi topla" bir yorumdur.

R komutlarını doğrudan konsolda da çalıştırabilirsiniz. Gönderiminiz doğruluğu kontrol edilmediğinden, R kodu ile deneme yapmak için iyi bir yoldur.

`@instructions`
- Sağdaki editörde zaten bazı örnek kod var. Hangi satırların gerçek R kodu olduğunu ve hangi yorumları olduğunu görebiliyor musunuz?
- 5 ve 2 toplamını hesaplayan bir kod satırı ekleyin ve 'Cevabı Gönder' düğmesine basın.

`@hint`
Sadece örnek kodda olduğu gibi 5 ve 2 toplamını hesaplayan bir R kodu satırı ekleyin!
...
`@sct`
```{r}
test_output_contains("7", incorrect_msg = "Yeni bir satıra '5 + 2' eklediğinizden emin olun. Satırı bir `#` ile başlatmayın, aksi takdirde R kodunuz uygulanmaz!")
success_msg("Müthiş! Konsolun, gönderdiğiniz R kodunun sonucunu nasıl gösterdiğine bakın. Artık arayüze aşina olduğunuza göre, R'a başlayalım!")
```
